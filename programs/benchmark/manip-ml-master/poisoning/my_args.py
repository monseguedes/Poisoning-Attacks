...
def setup_argparse(poisct, seed):
    ...
    parser.add_argument(
        "-d",
        "--dataset",
        # default="datasets/house-processed.csv",
        default="programs/benchmark/manip-ml-master/datasets/house-processed.csv",
    ...
    parser.add_argument(
        ...
        default=1,
        type=float,
        help="lambda value to use in poisoning;icml 2015",
    )
    parser.add_argument(
        "-n",
        "--epsilon",
        default=1e-3,
        type=float,
        help="termination condition epsilon;icml 2015",
    )
    parser.add_argument(
        "-a",
        "--eta",
        default=0.01,
        type=float,
        help="line search multiplier eta; icml 2015",
    )
    parser.add_argument(
        "-b",
        "--beta",
        default=0.05,
        type=float,
        help="line search decay beta; icml 2015",
    )
    parser.add_argument(
        "-i",
        "--sigma",
    ...
    parser.add_argument(
        "-vis", "--visualize", action="store_true", help="visualize dataset"
    )
