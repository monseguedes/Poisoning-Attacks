...
def objective_function(model, function):
    ...
    sum_square_errors = gp.quicksum(
        (
            gp.quicksum(
                model.weights_num[r] * model.x_train_num[k, r]
                for r in model.numfeatures_set
            )
            + gp.quicksum(
                gp.quicksum(
                    model.weights_cat[j, z] * model.x_train_cat[k, j, z]
...
def loss_function_derivative_num_weights(model, poisoned, function, s):
    ...
    train_samples_component = gp.quicksum(
        gp.quicksum(
            model.weights_num[r] * model.x_train_num[i, r]
            for r in model.numfeatures_set
        )
        * model.x_train_num[i, s]
        + gp.quicksum(
            gp.quicksum(
                model.weights_cat[j, z] * model.x_train_cat[i, j, z]
                for z in range(1, model.no_categories[j] + 1)
            )
            for j in model.catfeatures_set
        )
    ...
    poison_samples_component = gp.quicksum(
        ...
        + gp.quicksum(
            gp.quicksum(
                model.tcn_lc_times_numsamples[k, j, z, s]
                for z in range(1, model.no_categories[j] + 1)
            )
            for j in model.catfeatures_set
        )
...
def loss_function_derivative_cat_weights(model, poisoned, function, l, h):
    ...
    poison_samples_component = gp.quicksum(
        ...
        + gp.quicksum(
            gp.quicksum(
                model.tcc_lc_times_catsample[k, j, z, l, h]
                for z in range(1, model.no_categories[j] + 1)
            )
            for j in model.catfeatures_set
        )
...
def loss_function_derivative_bias(model, function):
    ...
    poison_samples_component = gp.quicksum(
        ...
        + gp.quicksum(
            gp.quicksum(
                model.lc_catweight_times_catsample[k, j, z]
                for z in range(1, model.no_categories[j] + 1)
            )
            for j in model.catfeatures_set
        )
...
def ridge_objective_function(model, function):
    ...
    regularization_component = (
        ...
        * 1
        / 2
        * gp.quicksum(model.weights[r] ** 2 for r in model.features_set)
    )
