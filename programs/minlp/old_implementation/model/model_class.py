...
class PoisonAttackModel:
    ...
    def __init__(
        self,
        m: gp.Model,
        instance_data: model.instance_class.InstanceData,
        function="MSE",
        **kwds,
    ):  # Use initialisation parameters from AbtractModel class
        """
    ...
    def build_parameters(self, instance_data: model.instance_class.InstanceData):
        ...
        self.chosen_numfeatures = (
            instance_data.chosen_numerical
        )  # List of chosen numerical
        self.chosen_catfeatures = (
        ...
        if instance_data.numerical_columns == []:
            ...
            self.numfeatures_set = [
                int(column) for column in instance_data.numerical_columns
            ]
        self.catfeatures_set = sorted(
            [int(column) for column in list(instance_data.no_categories_dict)]
        )  # Set of categorical features
        self.no_categories = (
            instance_data.no_categories_dict
        )  # Depends on categorical features
        self.chosen_numfeatures = (
            instance_data.chosen_numerical
        )  # List of chosen numerical
        self.chosen_catfeatures = (
    ...
    def build_variables(self, instance_data: model.instance_class.InstanceData):
        ...
        self.x_poison_num = self.model.addVars(
            self.psamples_set,
            self.chosen_numfeatures,
            ...
            ub=1,
            name="x_poison_num",
        )  # , start=instance_data.num_x_poison_dataframe.to_dict())
        ...
        scc_indices = [
            (
                ...
                cat_feature,
                category,
            )  # Index of variables for categorical features
        ...
        self.x_poison_cat = self.model.addVars(
            scc_p_indices, vtype=GRB.BINARY, name="x_poison_cat"
        )
        ...
        self.weights_num = self.model.addVars(
            self.numfeatures_set,
            vtype=GRB.CONTINUOUS,
            lb=self.lower_bound,
            ub=self.upper_bound,
        ...
        cc_indices = [
            (cat_feature, category)  # Index for categorical weights
            for cat_feature in self.catfeatures_set
            for category in range(1, self.no_categories[cat_feature] + 1)
        ]
        self.weights_cat = self.model.addVars(
            cc_indices,
            vtype=GRB.CONTINUOUS,
            lb=self.lower_bound,
            ub=self.upper_bound,
            name="self.weights_cat",
        )

        self.bias = self.model.addVar(
            vtype=GRB.CONTINUOUS,
            lb=self.lower_bound,
            ub=self.upper_bound,
            name="bias",
        ...
        sccn_indices = [
            (sample, cat_feature, category, num_feature)
            for sample in self.psamples_set
        ...
        sncc_indices = [
            (sample, num_feature, cat_feature, category)
            for sample in self.psamples_set
            for num_feature in self.numfeatures_set
            for cat_feature in self.catfeatures_set
            for category in range(1, self.no_categories[cat_feature] + 1)
        ...
        scccc_indices = [
            (sample, cat_feature, category, catconstraint, categorycontraint)
            for sample in self.psamples_set
            for cat_feature in self.catfeatures_set
            for category in range(1, self.no_categories[cat_feature] + 1)
        ...
        self.tnn_ln_times_numsamples = self.model.addVars(
            ...
            lb=self.lower_bound,
            ub=self.upper_bound,
            name="tnn_ln_times_numsamples",
        )
        self.tcn_lc_times_numsamples = self.model.addVars(
            sccn_indices,
            lb=self.lower_bound,
            ub=self.upper_bound,
            name="tcn_lc_times_numsamples",
        )
        self.tnc_ln_times_catsamples = self.model.addVars(
            sncc_indices,
            lb=self.lower_bound,
            ub=self.upper_bound,
            name="tnc_ln_times_catsamples",
        )
        self.tcc_lc_times_catsample = self.model.addVars(
            scccc_indices,
            lb=self.lower_bound,
            ub=self.upper_bound,
            name="tcc_lc_times_catsample",
        )

        # Bilinear terms
        self.ln_numweight_times_numsample = self.model.addVars(
            self.psamples_set,
            self.numfeatures_set,
            lb=self.lower_bound,
            ub=self.upper_bound,
            name="ln_numweight_times_numsample",
        )
        self.lc_catweight_times_catsample = self.model.addVars(
            scc_indices,
            lb=self.lower_bound,
            ub=self.upper_bound,
            name="lc_catweight_times_catsample",
        )
    ...
    def build_constraints(self, trilinear_envelopes=False):
        ...
        num_gen = tuple(
            ...
            if numfeature not in self.chosen_numfeatures
        )
        cat_gen = tuple(
            catfeature
            for catfeature in self.catfeatures_set
            if catfeature not in self.chosen_catfeatures
        )
        ...
        for psample in self.psamples_set:
            for catfeature in self.chosen_catfeatures:
                self.model.addConstr(
                    aux.SOS_constraints(self, psample, catfeature) == 1,
                    name="SOSconstraints[%s,%s]" % (psample, catfeature),
                )

        print("Building num weights contraints")
        for numfeature in num_gen:
            self.model.addConstr(
                aux.loss_function_derivative_num_weights(
                    self, False, self.function, numfeature
                )
                == 0,
                name="cons_first_order_optimality_conditions_num_weights[%s]"
                % (numfeature),
            )
        for numfeature in self.chosen_numfeatures:
            self.model.addConstr(
                aux.loss_function_derivative_num_weights(
                    self, True, self.function, numfeature
                )
                == 0,
                name="cons_first_order_optimality_conditions_pnum_weights[%s]"
                % (numfeature),
            )
        ...
        for catfeature in cat_gen:
            for category in range(1, self.no_categories[catfeature] + 1):
                self.model.addConstr(
                    aux.loss_function_derivative_cat_weights(
                        self, False, self.function, catfeature, category
                    )
                    == 0,
                    name="cons_first_order_optimality_conditions_cat_weights[%s,%s]"
                    % (catfeature, category),
                )
        for catfeature in self.chosen_catfeatures:
            for category in range(1, self.no_categories[catfeature] + 1):
                self.model.addConstr(
                    aux.loss_function_derivative_cat_weights(
                        self, True, self.function, catfeature, category
                    )
                    == 0,
                    name="cons_first_order_optimality_conditions_pcat_weights[%s,%s]"
                    % (catfeature, category),
                )

        print("Building bias constraints")
        self.model.addConstr(
            aux.loss_function_derivative_bias(self, self.function) == 0,
            name="cons_first_order_optimality_conditions_bias",
        )
        ...
        for k in self.psamples_set:
            for r in self.numfeatures_set:
                for s in num_gen:
                    self.model.addConstr(
                        self.ln_numweight_times_numsample[k, r]
                        * self.x_data_poison_num[k, s]
                        == self.tnn_ln_times_numsamples[k, r, s],
                        name=f"cons_tnn[{k},{r},{s}]",
                    )
                for s in self.chosen_numfeatures:
                    self.model.addConstr(
                        self.ln_numweight_times_numsample[k, r]
                        * self.x_poison_num[k, s]
                        == self.tnn_ln_times_numsamples[k, r, s],
                        name=f"pons_tnn[{k},{r},{s}]",
                    )
                ...
                for z in range(1, self.no_categories[j] + 1):
                    for s in num_gen:
                        self.model.addConstr(
                            ...
                            * self.x_data_poison_num[k, s]
                            == self.tcn_lc_times_numsamples[k, j, z, s],
                            name=f"cons_tcn[{k},{j},{z},{s}]",
                        )
                    for s in self.chosen_numfeatures:
                        self.model.addConstr(
                            self.lc_catweight_times_catsample[k, j, z]
            ...
            for r in self.numfeatures_set:
                for l in cat_gen:
                    for h in range(1, self.no_categories[l] + 1):
                        self.model.addConstr(
                            self.ln_numweight_times_numsample[k, r]
                            * self.x_data_poison_cat[k, l, h]
                            == self.tnc_ln_times_catsamples[k, r, l, h],
                            name="cons_tnc",
                        )
                    ...
                    for l in cat_gen:
                        for h in range(1, self.no_categories[l] + 1):
                            self.model.addConstr(
                                self.lc_catweight_times_catsample[k, j, z]
                                * self.x_data_poison_cat[k, l, h]
                                == self.tcc_lc_times_catsample[k, j, z, l, h],
                                name="cons_tcc",
                            )
                    for l in self.chosen_catfeatures:
                        for h in range(1, self.no_categories[l] + 1):
        ...
        for k in self.psamples_set:
            for r in num_gen:  # Not poisoning, just data
                self.model.addConstr(
                    self.weights_num[r] * self.x_data_poison_num[k, r]
                    == self.ln_numweight_times_numsample[k, r],
                    name="cons_ln",
                )
            for r in self.chosen_numfeatures:  # Chosen poisoning
                self.model.addConstr(
                    self.weights_num[r] * self.x_poison_num[k, r]
                    == self.ln_numweight_times_numsample[k, r],
                    name="pcons_ln",
                )
        ...
        for k in self.psamples_set:
            ...
            for j in self.chosen_catfeatures:  # Chosen poisoning
                for z in range(1, self.no_categories[j] + 1):
                    self.model.addConstr(
                        self.weights_cat[j, z] * self.x_poison_cat[k, j, z]
                        == self.lc_catweight_times_catsample[k, j, z],
                        name="pcons_lc",
                    )
            ...
            for k in self.psamples_set:
                for r in self.numfeatures_set:
                    for s in self.numfeatures_set:
                        ...
                        self.model.addConstr(
                            self.tnn_ln_times_numsamples[k, r, s]
                            >= self.upper_bound * self.x_poison_num[k, s]
                            + self.upper_bound * self.x_poison_num[k, r]
                            + self.weights_num[r]
                            + 2 * self.lower_bound,
                            name="convex_envelope_1",
                        )
                        self.model.addConstr(
                            self.tnn_ln_times_numsamples[k, r, s]
                            >= self.lower_bound * self.x_poison_num[k, s],
                            name="convex_envelope_2",
                        )
                        self.model.addConstr(
                            self.tnn_ln_times_numsamples[k, r, s]
                            >= self.lower_bound * self.x_poison_num[k, r],
                            name="convex_envelope_3",
                        )
                        self.model.addConstr(
                            self.tnn_ln_times_numsamples[k, r, s]
                        ...
                        self.model.addConstr(
                            ...
                            + self.weights_num[r]
                            + 2 * self.upper_bound,
                            name="concave_envelope_1",
                        )
                        self.model.addConstr(
                            self.tnn_ln_times_numsamples[k, r, s]
                            <= self.upper_bound * self.x_poison_num[k, s],
                            name="concave_envelope_2",
                        )
                        self.model.addConstr(
                            self.tnn_ln_times_numsamples[k, r, s]
                            <= self.upper_bound * self.x_poison_num[k, r],
                            name="concave_envelope_3",
                        )
                        self.model.addConstr(
                            self.tnn_ln_times_numsamples[k, r, s]
    ...
    def build_objective(self):
        ...
        self.model.setObjective(
            aux.objective_function(self, self.function), GRB.MAXIMIZE
        )
...
class RegressionModel:
    ...
    def build_variables(self, instance_data: model.instance_class.InstanceData):
        ...
        self.weights = self.model.addVars(
            self.features_set,
            vtype=GRB.CONTINUOUS,
            lb=-GRB.INFINITY,
            ub=GRB.INFINITY,
    ...
    def build_objective(self):
        ...
        self.model.setObjective(
            aux.ridge_objective_function(self, self.function), GRB.MINIMIZE
        )
...
class BenchmarkPoisonAttackModel(pmo.block):
    def __init__(
        self,
        instance_data: model.pyomo_instance_class.InstanceData,
        function,
        **kwds,
    ):
        super().__init__(
    ...
    def build_parameters(self, instance_data: model.pyomo_instance_class.InstanceData):
        ...
        self.catfeatures_set = range(
            1, self.no_catfeatures + 1
        )  # Set of categorical features
        self.no_categories = (
            instance_data.no_categories_dict
        )  # Depends on categorical features
    ...
    def build_variables(self, instance_data: model.pyomo_instance_class.InstanceData):
        ...
        )  # Bias of the linear regresion model
    ...
    def build_constraints(self):
        ...
        for numfeature in self.numfeatures_set:
            self.cons_first_order_optimality_conditions_num_weights[
                ...
                body=paux.loss_function_derivative_num_weights(
                    self, numfeature, self.function
                ),
                rhs=0,
            )
        ...
        for cat_feature in self.catfeatures_set:
            for category in self.categories_sets[cat_feature]:
                self.cons_first_order_optimality_conditions_cat_weights[
                    ...
                    body=paux.loss_function_derivative_cat_weights(
                        self, cat_feature, category, self.function
                    ),
                    rhs=0,
                )
        ...
        self.cons_first_order_optimality_conditions_bias = pmo.constraint(
            body=paux.loss_function_derivative_bias(self, self.function), rhs=0
        )
    ...
    def build_objective(self):
        ...
        self.objective_function = pmo.objective(
            expr=paux.mean_squared_error(self, self.function),
            sense=pyo.maximize,
        )
    ...
    def build_instance(self, instance_data: model.pyomo_instance_class.InstanceData):
        ...
        data_input = {
            None: {"x_train": instance_data.x_train_dataframe.to_dict()}
