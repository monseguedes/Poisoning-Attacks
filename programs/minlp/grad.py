...
def main():
    ...
    X_cat_train = np.array(
        [[np.random.choice(x) for x in n_categories] for i in range(n_train)]
    )
    X_cat_poison = np.array(
        [[np.random.choice(x) for x in n_categories] for i in range(n_poison)]
    )
    ...
    run(
        ...
        y_train,
        X_num_poison,
        X_cat_poison_one_hot,
        y_poison,
        optimize_catogorical_features=False,
    )
...
def run(
    ...
    X_cat_poison_one_hot = torch.tensor(
        X_cat_poison_one_hot, requires_grad=optimize_catogorical_features
    )
    ...
    optimizer = torch.optim.SGD(
        parameters, lr=0.1, momentum=0.0, maximize=True
    )
    ...
    for i in range(50):
        ...
        theta = torch.linalg.solve(
            X.T @ X + regularisation_parameter * torch.eye(X.shape[1]), X.T @ y
        )
