...
class GDPoisoner(object):
    def __init__(
        ...
        # # Made up by Monse
        # self.x = np.asarray(self.trnx)
        # self.y = np.asarray(self.trny)
        self.x = self.trnx
        self.y = self.trny
    ...
    def poison_data(self, poisx, poisy, tstart, visualize, newlogdir):
        ...
        clf_init, lam_init = self.learn_model(
            np.asarray(self.trnx), np.asarray(self.trny), None
        )
        ...
        while True:
            ...
            # new_poisx = np.asarray(new_poisx)
            new_poisy = [None for a in poisy]
            ...
            if workerpool:  # multiprocessing
                for i, cur_pois_res in enumerate(
                    workerpool.map(self.poison_data_subroutine, pois_params)
                ):
    ...
    def plot_grad(self, clf, lam, eq7lhs, mu, min_x=0, max_x=1, resolution=0.1):
        ...
        for i in range(grid_x.shape[0]):
            ...
            z[i, 0], z[i, 1] = self.comp_grad_dummy(
                eq7lhs, mu, clf, lam, x[i, 0].reshape((1, 1)), x[i, 1]
            )
    ...
    def comp_grad_dummy(self, eq7lhs, mu, clf, lam, poisx, poisy):
        ...
        wxc, bxc, wyc, byc = self.compute_wb_zc(
            eq7lhs, mu, clf.coef_, m, self.samplenum, poisx
        )
    ...
    def poison_data_subroutine(self, in_tuple):
        ...
        # compute partials
        wxc, bxc, wyc, byc = self.compute_wb_zc(
            eq7lhs, mu, clf.coef_, m, self.samplenum, poisxelem
        )
        ...
        if self.opty:
            raise SystemExit("Optimize y not implemented")
            allattack = np.array(np.concatenate((attack, attacky), axis=1))
            allattack = allattack.ravel()
        else:
    ...
    def computeError(self, clf):
        ...
        pred = clf.predict(np.asarray(self.tstx))
        # vpred = clf.predict(self.vldx)
        vpred = clf.predict(np.asarray(self.vldx))
...
class LinRegGDPoisoner(GDPoisoner):
    ...
    def compute_wb_zc(self, eq7lhs, mu, w, m, n, poisxelem):
        ...
        eq7rhs = -(1 / n) * np.bmat(
            [[m, -np.matrix(poisxelem.T)], [np.matrix(w.T), np.matrix([-1])]]
        )
...
class LassoGDPoisoner(LinRegGDPoisoner):
    def __init__(
        ...
        self.initclf, self.initlam = self.learn_model(
            self.trnx, self.trny, None, lam=None
        )
    ...
    def comp_attack_trn(self, clf, wxc, bxc, wyc, byc, otherargs):
        (r,) = otherargs
        attackx, attacky = LinRegGDPoisoner.comp_attack_trn(
            self, clf, wxc, bxc, wyc, byc, otherargs
        )
...
class RidgeGDPoisoner(LinRegGDPoisoner):
    ...
    def learn_model(self, x, y, clf, lam=None):
        lam = 0.1
        clf = linear_model.Ridge(alpha=len(x) * lam, max_iter=10000)
