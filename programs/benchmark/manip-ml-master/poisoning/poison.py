...
def adaptive(X_tr, Y_tr, count):
    ...
    # return np.asarray(np.matrix(X_pois)),Y_pois
...
def randflip(X_tr, Y_tr, count):
    ...
    Y_pois = [
        1 if 1 - Y_tr[i] > 0.5 else 0 for i in poisinds
    ]  # this is the flip all the way implementation
    return np.matrix(X_tr[poisinds]), Y_pois
    # return np.asarray(np.matrix(X_tr[poisinds])), Y_pois
...
def randflipnobd(X_tr, Y_tr, count):
    ...
    print("Points selected: ", poisinds)
    Y_pois = [
        1 - Y_tr[i] for i in poisinds
    ]  # this is for validating yopt, not for initialization
...
def rmml(X_tr, Y_tr, count):
    ...
    # return np.asarray(np.matrix(allpoisx)),poisy.tolist()
...
def main(args):
    ...
    for initit in range(args.numinit):
        poisx, poisy = init(
            trainx, trainy, int(args.trainct * totprop / (1 - totprop) + 0.5)
        )
    ...
    for i in range(args.partct + 1):
        ...
        poisres, poisresy = poiser.poison_data(
            curpoisx, curpoisy, timestart, args.visualize, newlogdir
        )
        ...
        if args.rounding:
            ...
            rpoisedx, rpoisedy = (
                np.concatenate((trainx, roundx), axis=0),
                trainy + roundy,
            )
