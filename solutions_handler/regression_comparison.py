...
class ComparisonModel():
    ...
    def make_poisoned_predictions(self, bilevel_model: model.model_class.PoisonAttackModel, 
        ...
        self.pred_bilevel_y_train = np.repeat(self.bilevel_model.bias.X, len(self.bilevel_dataframe))
        ...
        for column in self.bilevel_dataframe.columns:
            if ':' not in column:
                ...
                self.pred_bilevel_y_train += self.bilevel_dataframe[column] * self.bilevel_model.weights_num[column_index].X
            else:
                column_index = [int(index) for index in column.split(':')]
                self.pred_bilevel_y_train += self.bilevel_dataframe[column] * self.bilevel_model.weights_cat[(column_index[0], column_index[1])].X
    ...
    def make_benchmark_predictions(self, benchmark_model: model.model_class.BenchmarkPoisonAttackModel, 
        ...
        self.pred_benchmark_y_train = np.repeat(pyo.value(self.benchmark_model.bias), len(self.benchmark_dataframe))

        # Take columns one by one, convert them to vector, and multiply them by the corresponding weights
        for column in self.benchmark_dataframe.columns:
            if ':' not in column:
                column_index = int(column)
                self.pred_benchmark_y_train += self.benchmark_dataframe[column] * self.benchmark_model.weights_num[column_index]._value
            else:
                column_index = [int(index) for index in column.split(':')]
                self.pred_benchmark_y_train += self.benchmark_dataframe[column] * self.benchmark_model.weights_cat[(column_index[0], column_index[1])]._value
    ...
    def make_non_poisoned_predictions(self, ridge_instance: model.instance_class.InstanceData,
        ...
        if self.datatype == 'train':
            ...
            self.ridge_dataframe = self.ridge_instance.ridge_x_train_dataframe.copy(deep=True).unstack()
        
        elif self.datatype == 'test':
            self.y = list(self.ridge_instance.test_y)
            self.ridge_dataframe = self.ridge_instance.test_ridge_x_train_dataframe.copy(deep=True).unstack()
        
        # Define vector of size rows of data_dataframe, and with bias in all terms
        self.pred_ridge_y_train = np.repeat(self.ridge_model.bias.X, len(self.ridge_dataframe))
        
        # Take columns one by one, convert them to vector, and multiply them by the corresponding weights
        for column in self.ridge_dataframe.columns:
            column_index = int(column)
            self.pred_ridge_y_train += self.ridge_dataframe[column] * self.ridge_model.weights[column_index].X
    ...
    def plot_actual_vs_pred(self, model_name):
        ...
        plt.savefig('plots/' + self.folder + file_name + '.pdf', transparent=True, bbox_inches = "tight")
    ...
    def plot_actual_vs_predicted_all(self):
        ...
        plt.savefig('plots/' + self.folder +
                     ...
                     str(int(self.bilevel_instance.poison_rate * 100)) + '_' + 
                     str(self.bilevel_instance.seed) +  
                     '.png')
        plt.show()
