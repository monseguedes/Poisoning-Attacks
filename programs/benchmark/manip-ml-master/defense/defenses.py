...
def ransacmodel(x, y, model, lam, counts):
    ...
    for count in counts:
        clfransac = lm.RANSACRegressor(
            allmodels[model], min_samples=count
        )  # allmodels[model])
        clfransac.fit(x, y)
        score = clfransac.score(
            x[clfransac.inlier_mask_], y[clfransac.inlier_mask_]
        )
        scores.append(score)
        if score > bestscore:
...
def trimclf(x, y, count, lam, model):
    ...
    while (
        sorted(inds) != sorted(newinds)
        and it < 400
        and lasterr - toterr > 1e-5
    ):
        newinds = inds[:]
        lasterr = toterr
        subx = x[inds]
...
def chenrotr(x, y, count, ks):
    ...
    bestcorrs = sorted(
        [(abs(j), i) for i, j in enumerate(corrs)], reverse=True
    )
...
def ronidefense(x, y, count, lam, model, trainsizes):
    ...
    for trainsize in trainsizes:
        ...
        sort_increases = sorted(
            enumerate(all_increases), key=lambda tup: tup[1]
        )
        ...
        if score > bestscore:
            bestclf, besttsize, bestcleaninds, bestscore = (
                clean_model,
                trainsize,
                clean_inds,
                score,
